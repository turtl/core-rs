...
impl<'a> AesGcm<'a> {
    pub fn new (key_size: KeySize, key: &[u8], nonce: &[u8], aad: &[u8]) -> AesGcm<'a> {
        ...
        assert!(nonce.len() == 12 || nonce.len() == 16);

        // AL - adding this block to handle situations where IV.len() != 12,
        // which is pretty much all of my cases.
        //
        // note that i'm not sure how SOUND this code is. i got here by
        // comparing the intermediate values for the IV to values in SJCL, but
        // somehow ended up skipping steps but having everything work. for
        // instance, in every other lib i picked through (sjcl, gcrypt, nettle)
        // it seems that the iv gets hashed once with the hash_key, [0u8; 16],
        // and the iv then AGAIN with hash_key, hashed_iv, and an array that has
        // the iv length in it. my only conclusion is that unlike the above libs
        // rust_crypto's ghash actually does this hashing internally so only one
        // hash round is needed instead of two.
        //
        // this works with all my test cases, although i'm now curious if my IVs
        // were anything other than 16 bytes, would this work?
        if nonce.len() == 16 {
            ...
            let mut ivbl = [0u8; 16];
            ivbl[15] = 128;
            let iv = Ghash::new(&hash_key).input_c(nonce).result();
